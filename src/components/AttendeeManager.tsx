...
function RowForm({ initial, onSave, onCancel }: RowFormProps) {
  ...
  const inputCls = 'w-full border border-gray-200 rounded-lg px-2.5 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white';
          ...
          <div className="flex items-center gap-2">
            {isDemo && (
              <span className="text-xs text-amber-600 bg-amber-50 border border-amber-200 px-2.5 py-1.5 rounded-lg font-medium">
                Demo — connect calendar to manage
              </span>
            )}
            <button
              disabled={isDemo}
              onClick={() => !isDemo && setShowImport((v) => !v)}
              className="inline-flex items-center gap-1.5 px-3 py-2 border border-gray-200 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
            ...
            <button
              disabled={isDemo}
              onClick={() => { if (!isDemo) { setAction({ type: 'add' }); setShowImport(false); } }}
              className="inline-flex items-center gap-1.5 px-3 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
            >
          ...
          <table className="min-w-full">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              ...
              {filtered.map((m) => {
                ...
                return (
                  <tr key={m.email} className={`group hover:bg-gray-50 transition-colors ${isDemo ? 'opacity-80' : ''}`}>
                    <td className="px-3 py-3 text-sm text-gray-800 font-medium">
                      {m.name || <span className="text-gray-400 italic">—</span>}
                    </td>
                    <td className="px-3 py-3 text-sm text-gray-500 font-mono">{m.email}</td>
                    <td className="px-3 py-3">
                    ...
                    <td className="px-3 py-3">
                      <div className={`flex items-center gap-1 transition-opacity ${isDemo ? 'opacity-0 pointer-events-none' : 'opacity-100 sm:opacity-0 sm:group-hover:opacity-100'}`}>
                        <button
